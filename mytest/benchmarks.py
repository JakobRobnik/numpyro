...
def load_moments(name):
    dirr = 'mytest/data/ground_truth/'
    m2 = jnp.load(dirr + name + '_m2.npy')
    varm2 = jnp.load(dirr + name + '_varm2.npy')
...
class STN():
    ...
    def __init__(self):
        ...
        self.m2 = jnp.ones(d)
        self.varm2 = 2 * jnp.ones(d)
            
    def target(self):
        numpyro.sample('x', dist.Normal(jnp.zeros(self.d), jnp.ones(self.d)))
        
    def prior(self, key):
        return vec_to_x(numpyro.sample('x', dist.Normal(jnp.zeros(self.d), jnp.ones(self.d)), rng_key = key))
